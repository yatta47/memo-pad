# STEP 1 指示文（注釈付き。画面に出す用）

凡例: `[情報]` = 人が用意した正しい情報 / `[指示]` = 設計判断を言語化した指示

---

`.claude/skills/case-card/SKILL.md` を作ってください。URLを1本受け取り、記事を読んで「AI活用事例カード」のJSONを1件書くスキルです。 `[指示]`

## 出力 `[指示]`
- `catalog/<id>.json` に1ファイル。既にあれば上書きしない。
- スキーマは `docs/case-schema.md` に従う。キー名・型・順序を変えない。 `[情報: スキーマ]`
...
作ったら、`https://aws.amazon.com/jp/blogs/news/179920-2/` で1件生成して、結果のJSONを表示してください。 `[指示]`
